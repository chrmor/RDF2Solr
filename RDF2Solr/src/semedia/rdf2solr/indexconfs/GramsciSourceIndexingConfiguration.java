...
public class GramsciSourceIndexingConfiguration extends Configuration {
	...
	private static final String[] GRAMSCI_INDEXING_QUERIES = {	
		...
		//GRAMSCI_PREFIXES + "select distinct ?uri ?field ?value where {?uri dcterms:type <http://purl.org/gramcsiproject/vocab/Nota>. ?r ?field ?uri. ?r rdfs:label ?value}"
	...
	static {
		...
		GRAMSCI_FACET_QUERIES.put("mentions_book_ss", GRAMSCI_PREFIXES + "select distinct ?uri ?value where { ?uri dct:type gramsci:Nota. ?text dct:isPartOf ?uri. ?text cito:discusses ?entity. ?entity rdf:type <http://dbpedia.org/ontology/Book>.?entity rdfs:label ?value. } ");
		...
		GRAMSCI_FACET_QUERIES.put("cited_by_ss", GRAMSCI_PREFIXES + "select distinct ?uri ?value where {?uri dct:type <http://purl.org/gramcsiproject/vocab/Nota>. ?text dct:isPartOf ?uri. ?entry <http://purl.org/spar/cito/cites> ?text. ?entry rdfs:label ?value.} ");		
		...
		GRAMSCI_FACET_QUERIES.put("topic_ss", GRAMSCI_PREFIXES + "select distinct ?uri ?value where {?uri dct:type <http://purl.org/gramcsiproject/vocab/Nota>. ?topic <http://purl.org/net/cito/isDiscussedBy> ?uri. ?topic rdf:type skos:Concept. ?topic rdfs:label ?value }");
	}
